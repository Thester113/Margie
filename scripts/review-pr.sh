...
if [ -n "$REVIEW_SKILL" ]; then
  ...
  PROMPT="Review merge request !$PR in this repo thoroughly. Check out its branch (glab mr checkout $PR; if it is checked out in another worktree, review there without changing it), read the MR description and the full diff (glab mr diff $PR), then the surrounding code each change touches: callers, tests, config, migrations, docs. Check correctness, edge cases and failure modes, security and tenancy, data changes and their reversibility, tests that cover the change (run the relevant ones), the repo's ADRs/CLAUDE.md conventions, and whether the description matches what the diff does. Leave specific, actionable comments with glab mr note (file:line, what's wrong, what to do). Then post ONE verdict note, exactly this first line so the merge gate can read it: 'Local review verdict: **approve** for commit <first 8 chars of the MR head sha>' or 'Local review verdict: **request_changes** for commit <sha8>', followed by what you checked and why. Approve only if it is genuinely ready to merge; a nit alone is not a blocker. Also run glab mr approve when you approve (it may be refused for the author's own MR; that is fine)."
else
