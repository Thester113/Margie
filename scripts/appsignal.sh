...
ask() { # ask "<prompt>"
  ...
  if [ -z "$out" ]; then
    echo "AppSignal didn't answer, dearie — if this is the first use, the OAuth sign-in hasn't been done yet: run 'claude' in Warp, then /mcp, pick appsignal, and authenticate with your work email." >&2
    return 1
  fi
  printf '%s\n' "$out"
}
