...
if [ -z "$ADDR" ] || [ -z "$PASS" ]; then
  echo "Gmail isn't configured yet — add gmail_address and gmail_app_password to ~/.margie/config.json (create an app password at myaccount.google.com/apppasswords)." >&2
  exit 1
fi
...
try:
    ...
    if not ids:
        print("No matching messages."); M.logout(); sys.exit(0)
    for i in ids:
    ...
    print(f"Gmail login/read failed: {e}. If your Workspace blocks app passwords/IMAP, that's the cause.", file=sys.stderr); sys.exit(1)
except Exception as e:
    print(f"Gmail error: {e}", file=sys.stderr); sys.exit(1)
PY
    ;;
  send)
