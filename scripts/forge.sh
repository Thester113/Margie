...
if [ "$FORGE" = "gitlab" ]; then
  case "$cmd" in
    ...
    projects)
      OUT="$(list_projects)"; [ -n "$OUT" ] && echo "$OUT" || echo "No projects found${ORG:+ in $ORG}." ;;
    mrs)
      which="${1:-review}"
      ...
      [ -n "$OUT" ] && echo "$OUT" || echo "No open merge requests ($which)${ORG:+ in $ORG}." ;;
    mr)
      n="${1:-}"; [ -z "$n" ] && { echo "usage: forge.sh mr <n> [repo]" >&2; exit 1; }
      spec="$(repo_spec "${2:-}")" || { echo "Couldn't resolve repo '${2:-}'." >&2; exit 1; }
      glab api "projects/$(urlenc "$spec")/merge_requests/$n" | jq -r '
        ...
        || echo "Couldn't find MR !$n in $spec." ;;
    pipelines|ci)
      spec="$(repo_spec "${1:-}")" || { echo "Couldn't resolve repo '${1:-}'." >&2; exit 1; }
      glab api "projects/$(urlenc "$spec")/pipelines?per_page=${2:-5}" | jq -r '.[]? | "#\(.id) \(.status) on \(.ref) (\(.updated_at))"' \
        || echo "No pipelines for $spec." ;;
  ...
  case "$cmd" in
    ...
    projects) OUT="$(list_projects)"; [ -n "$OUT" ] && echo "$OUT" || echo "No repos found${ORG:+ in $ORG}." ;;
    mrs)
      which="${1:-review}"
      case "$which" in
      ...
      [ -n "$OUT" ] && echo "$OUT" || echo "No open pull requests ($which)${ORG:+ in $ORG}." ;;
    mr|pr)
      n="${1:-}"; [ -z "$n" ] && { echo "usage: forge.sh mr <n> [repo]" >&2; exit 1; }
      spec="$(repo_spec "${2:-}")" || { echo "Couldn't resolve repo '${2:-}'." >&2; exit 1; }
      gh pr view "$n" -R "$spec" --json number,title,author,state,isDraft,headRefName,baseRefName,reviewDecision,url \
        ...
        || echo "Couldn't find PR #$n in $spec." ;;
    pipelines|ci)
      spec="$(repo_spec "${1:-}")" || { echo "Couldn't resolve repo '${1:-}'." >&2; exit 1; }
