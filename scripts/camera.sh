...
if [ -s "$OUT" ]; then
  ...
  echo "Camera capture failed, dearie — check Camera permission for Margie (System Settings → Privacy & Security → Camera), and that the panel camera isn't already using it."
  exit 1
fi
