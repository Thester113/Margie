...
if [ -z "$EMAIL" ] || [ -z "$TOKEN" ] || [ -z "$BASE" ]; then
  echo "Jira isn't configured yet, sir — add atlassian_email, atlassian_api_token and jira_base_url to ~/.margie/config.json." >&2
  exit 1
fi
...
case "$cmd" in
  ...
  mine)
    ...
    api GET "/rest/api/3/search/jql?maxResults=20&fields=summary,status&jql=$(jq -rn --arg j "$JQL" '$j|@uri')" \
      ...
      || echo "Couldn't list your issues, sir."
    ;;
  search)
    Q="$*"
  ...
  create)
    ...
    if [ -n "$NEWKEY" ]; then echo "Created $NEWKEY, sir: $BASE/browse/$NEWKEY";
    else echo "Create failed, sir: $(echo "$RESP" | jq -rc '.errors // .errorMessages // .' 2>/dev/null | head -c 200)"; fi
    ;;
  comment)
    ...
    if echo "$RESP" | jq -e '.id' >/dev/null 2>&1; then echo "Commented on $KEY, sir.";
    else echo "Comment failed, sir: $(echo "$RESP" | jq -rc '.errors // .errorMessages // .' 2>/dev/null | head -c 200)"; fi
    ;;
  *)
