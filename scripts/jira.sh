...
if [ -z "$EMAIL" ] || [ -z "$TOKEN" ] || [ -z "$BASE" ]; then
  echo "Jira isn't configured yet — add atlassian_email, atlassian_api_token and jira_base_url to ~/.margie/config.json." >&2
  exit 1
fi
...
case "$cmd" in
  ...
  mine)
    ...
    api GET "/rest/api/3/search/jql?maxResults=20&fields=summary,status&jql=$(jq -rn --arg j "$JQL" '$j|@uri')" \
      ...
      || echo "Couldn't list your issues."
    ;;
  search)
    Q="$*"
  ...
  create)
    ...
    [ -z "$PROJ" ] && { echo "No project — pass one (jira.sh create PROJ \"...\") or set jira_default_project." >&2; exit 1; }
    ...
    if [ -n "$NEWKEY" ]; then echo "Created $NEWKEY: $BASE/browse/$NEWKEY";
    else echo "Create failed: $(echo "$RESP" | jq -rc '.errors // .errorMessages // .' 2>/dev/null | head -c 200)"; fi
    ;;
  comment)
    ...
    if echo "$RESP" | jq -e '.id' >/dev/null 2>&1; then echo "Commented on $KEY.";
    else echo "Comment failed: $(echo "$RESP" | jq -rc '.errors // .errorMessages // .' 2>/dev/null | head -c 200)"; fi
    ;;
  *)
