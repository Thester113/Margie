...
case "$cmd" in
  list) ls -t "$P" 2>/dev/null | sed 's/\.md$//' | while read -r n; do printf '%s  (%s)\n' "$n" "$(head -1 "$P/$n.md" | cut -c1-80)"; done; [ -z "$(ls -A "$P" 2>/dev/null)" ] && echo "No project notes yet." ;;
  show) x="${1:?name}"; f="$P/$x.md"; [ -f "$f" ] || f="$(ls "$P"/*.md 2>/dev/null | grep -i -- "$x" | head -1)"; [ -f "${f:-}" ] && cat "$f" || { echo "No project note matching '$x'." >&2; exit 1; } ;;
  add) n="${1:?name}"; shift; t="$*"; [ -z "$t" ] && { echo "usage: notes.sh add <name> \"<text>\"" >&2; exit 1; }; f="$P/$(printf '%s' "$n" | tr 'A-Z /' 'a-z--').md"
    ...
    echo "Noted under $(basename "$f" .md) (top of STATE)." ;;
