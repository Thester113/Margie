...
case "$cmd" in
  ...
  add) n="${1:?name}"; shift; t="$*"; [ -z "$t" ] && { echo "usage: notes.sh add <name> \"<text>\"" >&2; exit 1; }; f="$P/$(printf '%s' "$n" | tr 'A-Z /' 'a-z--').md"; [ -f "$f" ] || printf '# %s\n\n' "$n" > "$f"; printf -- '- %s: %s\n' "$(date +%F)" "$t" >> "$f"; echo "Noted under $(basename "$f" .md), dearie." ;;
